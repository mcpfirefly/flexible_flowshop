...
from src.experiments.rl_algorithms.unstable_baselines.common.agents import BaseAgent
from torchrl.modules.distributions import MaskedCategorical
from torch.distributions import Categorical
# from src.experiments.rl_algorithms.unstable_baselines.common.networks import  MLPNetwork, PolicyNetwork, get_optimizer
from src.experiments.rl_algorithms.unstable_baselines.common.networks import SequentialNetwork, JointNetwork, PolicyNetworkFactory, get_optimizer
import numpy as np
import gym
from src.experiments.rl_algorithms.unstable_baselines.common import util, functional
...
class SACAgent(BaseAgent):
    ...
    def select_action(self, obs, legal_actions, deterministic=False):
        ...
        bool_mask = torch.Tensor(legal_actions)
        bool_mask = torch.gt(bool_mask, 0).int()
        bool_mask = bool_mask.to(util.device)

        if len(obs.shape) in [1,3]:
            obs = [obs]
        if type(obs) != torch.tensor:
            obs = torch.FloatTensor(np.array(obs)).to(util.device)
        action, probs = itemgetter("action", "probs")(self.policy_network.sample(obs, deterministic=deterministic))
        masked_probs = probs * bool_mask + 1e-10
        #masked_probs = masked_probs/(masked_probs.sum())
        masked_probs = torch.flatten(masked_probs)
        actions_distributions = Categorical(probs=masked_probs)
