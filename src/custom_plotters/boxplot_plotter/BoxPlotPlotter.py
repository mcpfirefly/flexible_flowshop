...
def BoxPlotPlotter(data,filename,n_samples,generate_heuristic_schedules=None):
    ...
    filename_boxplot_arrays = "{}/{}_boxplot.csv".format(filename, generate_heuristic_schedules)
    ...
    for i, key in enumerate(data.keys()):
        ...
        BoxPlot = data.boxplot(key, ax=ax[i], return_type='dict', showmeans=True, meanline=True, medianprops=medianprops)
    ...
    if generate_heuristic_schedules != None:
        plt.suptitle("Box Plot - Experiment with Policy: {}, Samples = {}".format(generate_heuristic_schedules,n_samples), fontsize=15,y=0.945)
    else:
        plt.suptitle("Box Plot , Samples = {}",format(n_samples), fontsize=15,y=0.945)

    plt.savefig("{}/{}_Boxplot.png".format(filename, generate_heuristic_schedules), dpi=400)
    plt.close()
