...
class JobTypes(Enum):
    ...
    BLOCKING : int
        A constant representing a blocking job type.
...
class GanttPlotter:
    ...
    def _find_xticks(
        ...
        number_full_ticks = int(max_value // step_size + 1)  # Including Zero
    ...
    def _create_legend(self, color_mode: int, max_elements_per_column: int) -> Any:
        ...
        if color_mode == 0 or color_mode == 1:
            ...
            for type in JobTypes:
                ...
                elif type == JobTypes.BLOCKING:
                    element_label = "blocking"
                    element_color = blocking_color
    ...
    def _plot_jobs(
        ...
        label_changeovers: bool
            Whether or not to label the changeover times in the Gantt chart.
        """
    ...
    def _add_labels_to_bars(
        ...
        labels_changeover = [job.duration for job in job_list]
    ...
    def _add_changeovers_to_bars(
        ...
        labels = [job.duration for job in job_list]
        types = [job.job_type for job in job_list]
    ...
    def _generate_color_dict(self, mode: int = 0):
        ...
        processing_color = hsv_to_rgb([44 / 360, 0.70, 0.9])
        changeover_color = hsv_to_rgb([180, 0.1, 1])
        blocking_color = hsv_to_rgb([100, 0.2, 0.8])
        ...
        for count, job in enumerate(self._jobs):
            ...
            if mode == 0:  # Unique color for each job name
                ...
                elif job.job_type == JobTypes.BLOCKING:
                    new_color = blocking_color
                else:
                    if job.name in self._job_color_dict:
