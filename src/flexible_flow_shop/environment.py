...
from flexible_flow_shop.resources.functions.scheduling_functions import (get_impact_factor)
from flexible_flow_shop.resources.functions.scheduling_functions import (prepare_historical_and_flags)
from flexible_flow_shop.resources.functions.scheduling_functions import (update_time_arrays)
from flexible_flow_shop.resources.functions.scheduling_functions import current_variables
from flexible_flow_shop.resources.functions.scheduling_functions import past_variables
from flexible_flow_shop.resources.functions.scheduling_functions import flag_as_legals_in_next_stage, get_orders_next_stage
from flexible_flow_shop.resources.functions.scheduling_functions import assign_position_in_schedule
...
class flexible_flow_shop(gym.Env):
    ...
    def reset(self):
        ...
        self.legal_operations = [
            True if (order.stage == 0 and order.release_date == 0 and order.valid == 1) else False for order in
            self.orders]
    ...
    def _scheduling(self, env, action):
        ...
        if self.legal_operations[action] and action != len(ORDERS):
            ...
            if buffer_usage != "no_buffers" and order.stage > 0:
                ...
                if go_to_buffer and order.visited_buffer == False:
                    if len(self.factory.buffer[buffer_stage].users) == 0 or (
                            order.product_code == self.current_product_in_buffer[buffer_stage]
                        ...
                        self.current_product_in_buffer[buffer_stage] = order.product_code
                        ...
                        with self.factory.buffer[buffer_stage].request() as request:
                            ...
                            if len(self.history_buffer[buffer_stage]) > 1:
                                ...
                                if self.remaining_time > 0:
                                    past_order.time_end_changeover_buffer = self.env.now + self.remaining_time
                                ...
                                if order.product_code != past_order.product_code:
                                    ...
                                    self.time_until_operation_done[order.position_in_schedule] = order.buffer_changeover
                                    self.time_until_job_done[order.order_id] = order.buffer_changeover
                                    order.time_end_changeover_buffer = self.env.now + order.buffer_changeover
                                    ...
                                    while past_order.changeover_in_progress:
                                        self.remaining_time = past_order.time_end_changeover_buffer - self.env.now
                                        self.time_until_operation_done[order.position_in_schedule] = self.remaining_time
                                        self.time_until_job_done[order.position_in_schedule] = self.remaining_time
                            ...
                            # PROCESS IN BUFFER
                            self.time_until_operation_done[order.position_in_schedule] = order.time_in_buffer
                            self.time_until_job_done[order.order_id] = order.time_in_buffer

                            order.time_start_buffer = self.env.now
                            order.time_end_buffer = order.time_start_buffer + order.time_in_buffer
                            ...
                            initial_number_of_users =  len(self.factory.buffer[buffer_stage].users)
                            while self.legal_machines[order.machine_id] == False:
                                if len(self.factory.buffer[buffer_stage].users) > initial_number_of_users and order.time_end_block == None:
                                    order.time_end_block = self.env.now
                                yield env.timeout(0.01)
                            intermediate_number_of_users = len(self.factory.buffer[buffer_stage].users)
                            if order.time_end_block == None:
                                order.time_end_block = self.env.now
                            block_duration = order.time_end_block - order.time_start_block
                            ...
                            if order.time_end_block != order.time_start_block:
                                if len(self.factory.buffer[buffer_stage].users) == 1 or initial_number_of_users <= intermediate_number_of_users:
            ...
            if self.machine_queue[order.machine] < MAX_QUEUE_SIZE:
                ...
                with self.factory.stage[i].get(lambda machine: machine == order.machine) as request:
                    ...
                    self.current_operation_in_machine[order.machine_id] = order.operation_id
                    relative_stage = np.around(np.divide(order.stage, order.total_stages), 4)
                    ...
                    order.waiting_value = np.around((order.time_start_process - order.time_arriving_stage), 4)
                    ...
                    self.jobs_completion[order.order_id] += np.divide(1,order.total_stages)
                    order.time_leaving_stage = self.env.now
                    order.processing_value = (order.time_leaving_stage - order.time_start_process)
                    #print("{} left stage {} at {}.".format(order.product_code, stage, order.time_leaving_stage))
                    self.sim_duration = env.now
                    self.time_left_previous_stage[order.order_id] = order.time_leaving_stage
                    ...
                    if generate_heuristic_schedules == "FIFO" and order.stage != len(STAGES) - 1:
                        ...
                        while order.position_heuristics != self.counter_positions_heuristics[order.next_stage]:
                            yield env.timeout(0.001)
                    ...
                    if (np.around(self.jobs_completion[order.order_id] * order.total_stages, 2) == order.total_stages):
                        ...
                        if order.earliness < 0:
                            order.earliness = 0
                        order.weighted_lateness = (0.9 * order.earliness + 4.5 * order.tardiness)
                    ...
                    if changeover_condition:
                        ...
                        while len(self.historical_selection[order.machine]) == 0:
                            ...
                            self.history_default_changeover[order.position_in_schedule] += default_changeover_time
                        ...
                        if (order.changeover_value > self.history_default_changeover[order.position_in_schedule]):
                            remaining_changeover_time = (order.changeover_value - self.history_default_changeover[order.position_in_schedule])
                            self.time_until_machine_free[order.machine_id] = remaining_changeover_time
                            self.time_until_operation_done[order.position_in_schedule] = remaining_changeover_time
                            self.time_until_job_done[order.order_id] = remaining_changeover_time
                            yield env.timeout(remaining_changeover_time)

                        order.time_changeover_ends = self.env.now
                        order.changeover_cost = (order.changeover_value * order.impact_factor_value)
                    ...
                    yield self.factory.stage[i].put(order.machine)  # Put until next order is ready to start processing
    ...
    def step(self, action):
        ...
        if action_space == "continuous" and not(self.legal_operations[action]):
            ...
            if self.finished_orders.all() or np.round(self.completion_score,5) == 1.0:
                print("------------------------------------------------------")
                print("Episode terminated, good job! Completion score: {}%".format(self.completion_score*100))
            ...
            if self.sim_duration >= 60 or self.episode_length >= 5000:
                print("------------------------------------------------------")
                print("Episode truncated! Completion score: {}%".format(self.completion_score * 100))
                print("Episode length: {}".format(self.episode_length))
            ...
            self.makespan_reward = -(np.around(self.sim_duration_current,4) - np.around(self.sim_duration_past,4))
            self.occ_reward = -(np.around(self.oc_costs_current,4) - np.around(self.oc_costs_past,4))
    ...
    def render(self):
        ...
        ylabel="",
        description= "Makespan = {}, OCC = {}, WL = {}".format(episode_makespan,episode_oc_costs,episode_wl),
