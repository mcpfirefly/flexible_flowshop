...
class CustomMaskableEvalCallback(MaskableEvalCallback):
    def __init__(self, eval_env, n_eval_episodes, eval_freq, best_model_save_path, verbose=0,log_path=None):
        super().__init__(eval_env, n_eval_episodes=n_eval_episodes, eval_freq=eval_freq, log_path=log_path,
    ...
    def _on_step(self):
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            self.logger.record("advanced_eval/makespan", np.float32(self.eval_env.buf_infos[0]["sim_duration"]))
            self.logger.record("advanced_eval/oc_costs", np.float32(self.eval_env.buf_infos[0]["oc_costs"]))
            self.logger.record("advanced_eval/weighted_lateness", np.float32(self.eval_env.buf_infos[0]["weighted_lateness"]))
            self.logger.record("advanced_eval/total_waiting", np.float32(self.eval_env.buf_infos[0]["total_waiting"]))
            self.logger.record("advanced_eval/completion_score", np.float32(self.eval_env.buf_infos[0]["completion_score"]))
            self.logger.record("advanced_eval/total_reward", np.float32(self.eval_env.buf_infos[0]["total_reward"]))
        return continue_training

class CustomEvalCallback(EvalCallback):
    def __init__(self, eval_env, n_eval_episodes, eval_freq, best_model_save_path, verbose, deterministic,log_path=None):
        super().__init__(eval_env, n_eval_episodes=n_eval_episodes, eval_freq=eval_freq,log_path=log_path,
                         best_model_save_path=best_model_save_path, verbose=verbose, deterministic=deterministic)
...
class RewardWrapper_MAKESPAN(gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        modified_reward = -(np.around(self.sim_duration_current,4) - np.around(self.sim_duration_past,4))
...
class ActionWrapper_Continuous(gym.ActionWrapper):
    ...
    def action(self, action):
        if (self.action_space.low == -self.action_space.high).all():
            action = (action + self.action_space.high) / (self.action_space.high - self.action_space.low)  # transform normalized action vector [-1,1] to [0,1]
...
class ProbabilitiesActionMaskEnv(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        action_mask = torch.from_numpy(self.env.valid_action_mask()).float()
        masked_action = torch.from_numpy(action).float() * action_mask

        normalized_probs = np.array(masked_action / masked_action.sum())
