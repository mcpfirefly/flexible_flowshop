...
def generate_subplots_for_custom_plots(file_paths, column_x, column_y, is_evaluation=False):
    ...
    plt.savefig(os.path.join(base_source_directory,
                             f'{experiment_name}_{plot_type}_{format_column(column_x).replace(" ", "_")}_vs_{format_column(column_y).replace(" ", "_")}.svg'),
                dpi=400, transparent=True, format="svg")
